...
class StockPicking(models.Model):
    ...
    ready_to_deliver = fields.Boolean(related='sale_id.ready_to_deliver', store=True, 
        string = 'To Deliver', 
        help='Indicates if can be delivered')

    @api.depends('state')
    def _compute_show_validate(self):
        super(StockPicking, self)._compute_show_validate()
        for picking in self:
            if picking.sale_id and picking.show_validate and not picking.ready_to_deliver:
